...
jQuery(function($) {	
	...
	$('body').on('click','.twilio-call', function(e) {
		e.preventDefault();
	...
	$('body').on('click','#vbx-client-status .client-button', function(e) {
		e.preventDefault();
		e.stopPropagation();
...
jQuery(function($) {
	...
	$(dialog).on('keypress','textarea', updateCount);
	$(dialog).on('keyup','textarea', updateCount);
	$(dialog).on('load','textarea', updateCount);
