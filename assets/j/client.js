...
$(function () {
	...
	$('#client-ui-answer', dialer).on('click', function(event) {
		stopEvent(event);
		Client.answer();
	});

	// Hangup Call button clicked
	$('#client-ui-hangup, #client-ui-close', dialer).on('click', function(event) {
		stopEvent(event);
		Client.hangup();
	});

	// Mute Call button clicked
	$('#client-ui-mute', dialer).on('click', function(event) {
		stopEvent(event);
		Client.togglemute();
	});

	// Button on Keypad clicked
	$(dialer).on('click','.client-ui-button', function(event) {
		stopEvent(event);
	...
	// Dialer tab clicked
	$(dialer).on('click','.client-ui-tab-wedge a, .client-ui-tab-status-inner', function(event) {
		stopEvent(event);
		if (Client.isReady()) {
			Client.ui.toggleCallView('close');
		}
	...
	$('#make-call-form', dialer).on('submit', function(event) {
		stopEvent(event);
		Client.makeCallTo($('#dial-phone-number').val());
	});
	$('#dial-input-button', dialer).on('click', function(event) {
		stopEvent(event);
		$('#make-call-form').submit();
	});
	
	// Dial button on user list clicked
	$( dialer).on('click','.user-dial-button', function(event) {
		stopEvent(event);
	...
	$('#client-mode-status a', dialer).on('click', function(event) {
		stopEvent(event);
	...
	$('#call-options-summary').on('click', function(e) {
		stopEvent(e);
		Client.ui.toggleOptionsSummary(this);
	});
	
	$('#call-options-inputs :input').on('change', function(e) {
