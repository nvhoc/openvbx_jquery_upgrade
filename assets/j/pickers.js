...
$(document).ready(function() {
    ...
    if($.browser.msie) {
        ...
        // When someone clicks one of the [read text, upload, record, library] items from
        // the selector, we should open the editor for that input type.
        $('body').on('click','.audio-choice-selector a', Pickers.audio.selectAction);
        // When someone cancels the editor, we should return to the input type selector
        $('body').on('click','.audio-choice-editor .audio-choice-close-button', Pickers.audio.closeEditorsAndShowSelector);
        // When someone saves their new "read text" setting, we should save and then show the newly saved value
        $('body').on('click','.audio-choice-editor .audio-choice-read-text button[type="submit"]', Pickers.audio.saveReadText);
        // When someone wants clicks edit on the current value, we should show the input selector guy
        $('body').on('click','.audio-choice-current-value .change', Pickers.audio.showInputSelector);
        // Whene someone closes the selector, we should go back to the current value
        $('body').on('click','.audio-choice-selector .audio-choice-close-button', Pickers.audio.closeSelectorAndShowCurrentValue);
        // When someone clicks the call & record button, they should get a phone call
        $('body').on('click','.audio-choice-editor .audio-choice-record button[type="submit"]', Pickers.audio.startDeviceRecording);
        // When someone is on the library, and they've set one of the recordings to be the active recording, we should save it and go back to the input selector
        $('body').on('click','.audio-choice-editor .audio-choice-library button[type="submit"]', Pickers.audio.setRecordingFromLibarary);
        
        // When someone hovers over the flash upoader, we want to change the background color that's
        // _behind_ the flash widget 
        $('body').on('mouseenter','.audio-choice-editor .swfupload-control', function(e) {
            $(e.target).closest('.audio-choice').find('.swfupload-container').addClass('hover');
        });
        $('body').on('mouseleave','.audio-choice-editor .swfupload-control', function(e) {
            $(e.target).closest('.audio-choice').find('.swfupload-container').removeClass('hover');
        });

        $('body').on('click','.usergroup-picker', Pickers.usergroup.dialog);
        $('body').on('mouseover','.usergroup-dialog td', Pickers.usergroup.hoverIn)
            .on('mouseout', Pickers.usergroup.hoverOut);
        $('body').on('click','.usergroup-dialog td', Pickers.usergroup.select);
        
        $('body').on('click','.usergroup-dialog a.edit-user',Pickers.usergroup.editUser);
        $('body').on('click','.usergroup-dialog a.edit-group', Pickers.usergroup.editGroup);
    }
});
